...
predictor = TabularPredictor(label="SalePrice",eval_metric='root_mean_squared_error').fit(data, presets="good_quality",time_limit=1000)
...
sub.to_csv('auto__medium_house.csv', index=False)
...
# plt.title('True vs Predicted SalePrice(auto)')

# # 绘制对角线（y=x），表示完美预测的情况
# plt.plot([min(y_true), max(y_true)], [min(y_true), max(y_true)], linestyle='--', color='red')
# plt.grid('--')
# plt.savefig("autoML.jpg",dpi=400)
# plt.show()
